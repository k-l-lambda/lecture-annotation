...
## 23.2 巨大的多粒子系统态空间 { .tutor-section }
...
## 23.5 哈迪的 EPR 事例：几乎与概率无关 { .tutor-section }
...
因此，我们需要 $\frac{1}{6}(10 \times 11 \times 12) = 220$ 个复数来定义态：即 $\mathbf{H}^{220}$ 而非 $\mathbf{H}^{1000}$ 里的一个元素。对 $n$ 个全同玻色子，这个数字是 $(9+n)!/9!\ n!$，即有这么多个独立复数 $z_{ij\ldots m}$，它们关于指标对称（记号见 §§ 12.4, 7 和 [§14.7](chapter_14.md#147-度规能为你做什么)）：
...
## 23.8 玻色子和费米子的量子态 { .tutor-section }
...
这个地方用 [§23.3](#233-量子纠缠贝尔不等式) 所述的态的（抽象）指标记号（也可以用 [§12.8](chapter_12.md#128-张量抽象指标记法和图示记法) 描述的图示记号，见[图 26.1](assets/page493_fig01.jpg)）最合适了。对此我们可以用记号 $\psi^\alpha$ 表示特定粒子（用上标 $\alpha$ 表示）的波函数，$\phi^\beta$ 表示第二个粒子（用上标 $\beta$ 表示）的波函数，等等。如果粒子不全同，则这一对粒子的波函数为（张量）积态：
...
量子隐形传态的特别惊人之处在于，仅仅通过发给我的同事两比特的经典信息（0，1，2，3中的任意一个数字可分别编码为00，01，10，11），我就传递了整个黎曼球面上某一点的"信息"（见[图22.14](assets/page422_fig01.jpg)）。按一般的经典概念，要做到这一点，我们需要有在连续统当中自由选取一点的信息量：严格来说就是$\aleph_0$的信息量（见[§16.3](chapter_16.md#163-无限的不同大小)，4）！我们是怎么做到这一点的呢？这里我要指出，已经有真实的实验确认了量子力学隐形传态的期望值（当然距离不是地球—土星这种跨度量级上的），[14]因此我们必须认真对待这些事儿。不仅如此，日益繁荣的量子加密技术也依赖于这种传递的普遍性质，许多量子计算概念也是如此。

我们来看一下[图23.7](assets/page451_fig01.jpg)。这幅时空图画出了我自己的、我朋友的和我的同事的世界线、所有相关粒子的世界线，还包括了我发给在土卫六的同事的经典信号。尽管实际上只传递了两比特的信息，朋友的粒子的自旋方向（标为$|\!\swarrow\rangle$）的"信息"就传到了土卫六。所有其他$\aleph_0$比特的信息是
