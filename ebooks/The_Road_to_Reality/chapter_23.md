...
??? question "答案 [23.7]"
    把每个二态系统表示为黎曼球面上的一点。若两测量方向夹角为 $\theta$，相应旋量态的内积模平方为 $\cos^2(\theta/2)$，与正交态的内积模平方为 $\sin^2(\theta/2)$。

    单态保证两边沿同一方向完全反相关。因此若一边测得某方向的“上”，另一边态塌缩为反方向；再与另一测量轴比较，就得到正文中的 $\cos^2(\theta/2)$ 和 $\sin^2(\theta/2)$ 概率公式。
