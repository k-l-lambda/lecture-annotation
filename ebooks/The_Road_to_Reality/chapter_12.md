...
怎么来刻画流形 $\mathcal{M}$ 呢？我们将它看作是许多坐标拼块的“粘合”。这里每个拼块都是 $\mathbb{R}^n$ 的一个开区域，$\mathbb{R}^n$ 代表“坐标空间”，其中的点是 $n$ 元组实数 $(x^1,\ x^2,\ \cdots,\ x^n)$，大家一定还记得（[§6.1](chapter_06.md#61-如何构造实函数)）$\mathbb{R}$ 代表的是实数系。在粘合过程中，我们有所谓转移函数，它将某个拼块的坐标表示为其他拼块坐标的函数。坐标拼块之间的重叠可以在流形 $\mathcal{M}$ 的任何地方找到。这些转移函数必须满足一些约束条件以保证整个粘合过程的协调性。粘合过程如[图 12.5](assets/page178_fig01.jpg)(a) 所示。为了生成标准流形，^4^ 即所谓豪斯道夫空间，我们得格外小心。（非豪斯道夫流形可以是“分支”，如[图 12.5](assets/page178_fig01.jpg)(b) 和[图 8.2](assets/page114_fig01.jpg)(c) 所示。）豪斯道夫空间有明确的属性：对空间上两个相异的点，存在包含每一点的开集，这些开集彼此不相交（[图 12.5](assets/page178_fig01.jpg)c）。
...
![图12.5 (a) 坐标拼块在三重重叠区域的粘合示意图；(b) 出现非豪斯道夫空间特征"分支"的流形示意图；(c) 豪斯道夫空间中相异两点具有彼此不重叠邻域的示意图。](assets/page178_fig01.jpg)
...
## 12.8　张量：抽象指标记法和图示记法 { .tutor-section }
