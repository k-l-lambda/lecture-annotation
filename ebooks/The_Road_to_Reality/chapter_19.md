...
这里，按[§14.7](chapter_14.md#147-度规能为你做什么)，$F_{ab}$ 的指标升 $F^{ab}$ 就是 $g^{ac}g^{bd}F_{cd}$。注意到 $\varepsilon^{abcd}=g^{ap}g^{bq}g^{cr}g^{ds}\varepsilon_{pqrs}$ 满足 $\varepsilon^{0123}=-1$，因此[§12.7](chapter_12.md#127-体积元求和规则)里的 $\mathbf{\varepsilon}$ 由 $\mathbf{\varepsilon}_{abcd}=-\mathbf{\varepsilon}^{abcd}$ 给定。*[19.1] 图19.1以图表方式给出了这些“对偶”运算（以及麦克斯韦方程本身）。我们将发现，这种意义（以及其他相关意义）下的“对偶”在各种不同
...
积分在闵可夫斯基空间M的任意三维闭曲面$\mathcal{Q}$上进行。（M的任意三维闭曲面是M的某个四维紧致区域$\mathcal{R}$的边界$\partial\mathcal{R}$。）见图19.3。量${}^{*}\mathbf{J}$可理解为穿过$\mathcal{Q} = \partial\mathcal{R}$的"电荷通量"（或电荷"流量"）。因此，上述方程告诉我们的是，穿过边界的净电荷通量为零；即所有流入$\mathcal{R}$的电荷量精确等于所有从$\mathcal{R}$流出的电荷量：电荷守恒。***[19.7]

我们还可以从麦克斯韦方程第二式$\mathrm{d} {}^{*}\mathbf{F} = 4\pi {}^{*}\mathbf{J}$导出所谓"高斯定理"。在某一时刻$t = t_0$应用这一定理，就可得到三维的外演算基本定理。它告诉我们，$t_0$时刻处于二维闭曲面$S$内的总电荷值（见图19.4），可以表为麦克斯韦张量${}^{*}\mathbf{F}$的对偶在$S$上的积分。或者说，通过积分穿过$S$的电场$\mathbf{E}$的总通量，我们可以获得$S$包围的总电荷量。***[19.8]
...
有意思的是，联络的路径依赖性（我们可以比较一下[图19.5](assets/page346_fig01.jpg)里的路径依赖性）表现为一种出人意表的实验现象，即所谓阿哈罗诺夫-玻姆（Aharonov-Bohm）效应。由于联络 $\mathbf{\nabla}$ 的作用只是一种量子现象，因此我们在经典实验上是看不到这种路径依赖性的。阿哈罗诺夫-玻姆效应有赖于量子干涉现象（见 [§21.4](chapter_21.md#214-量子理论的实验背景) 和[图21.4](assets/page384_fig01.jpg)）。在其典型实验里，电子被准直成束后由分束片分成两束通过一个无电磁场（$\mathbf{F}=0$）的区域，区域中两束电子路径之间置有一长直密绕螺管（管内充满磁力线，管外无磁场），最后两束电子在检测屏上形成干涉条纹（见图19.6(a)）。整个过程中电子都处于无场 $\mathbf{F}$ 作用状态。但相关的无场区 $\mathcal{R}$（自源始，分束绕过密绕螺管，直至屏）却不是单连通的，对于 $\mathcal{R}$ 外的场 $\mathbf{F}$，不存在一种规范选择使得在 $\mathcal{R}$ 内电磁势 $\mathbf{A}$ 处处为零。正是这种非单连通 $\mathcal{R}$ 内的非零势的存在——或更准确地说，$\mathcal{R}$ 内 $\mathbf{\nabla}$ 的路径依赖性——导致屏上干涉条纹出现位移。
...
事实上，干涉条纹位移效应并不依赖于 $\mathbf{A}$ 在局部的具体值（这也不可能，因为如上所述，$\mathbf{A}$ 不是一个局部可观测量），而是取决于 $\mathbf{A}$ 的某个非局域积分。这就是量 $\oint\!A$，环路取为 $\mathcal{R}$ 内的拓扑非平凡闭曲线。见图19.6(b)。由于 $\mathcal{R}$ 内 $\mathrm{d}A$ 为零（因为 $\mathcal{R}$ 内 $\mathbf{F}=0$），因此如果在 $\mathcal{R}$ 内沿
