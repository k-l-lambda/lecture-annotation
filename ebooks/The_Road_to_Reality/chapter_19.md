...
电磁场和电荷-电流密度（按闵可夫斯基当初使用的规范）分别综合成时空2形式 $\mathbf{F}$（称为麦克斯韦场张量）和时空矢量 $\mathbf{J}$（称为电荷-电流矢量），其分量的矩阵形式为
...
注意到2形式要求满足反对称 $F_{ba}=-F_{ab}$。我还将使用所谓 $\mathbf{F}$ 和 $\mathbf{J}$ 的霍奇对偶，它们分别是2形式 ${}^*\mathbf{F}$ 和3形式 ${}^*\mathbf{J}$，定义为
...
这里所需的反对称性质 ${}^*F_{ab}={}^*F_{[ab]}$ 和 ${}^*J_{abc}={}^*J_{[abc]}$ 均满足。根据列维-齐维塔张量 $\mathbf{\varepsilon}$（[§12.7](chapter_12.md#127-体积元求和规则)），并由整体反对称性 $\varepsilon_{abcd}$（$=\varepsilon_{[abcd]}$）和归一化条件，故有 $\varepsilon_{0123}=1$，于是这些对偶可写成

$${}^*F_{ab}=\frac{1}{2}\varepsilon_{abcd}F^{cd}\qquad\text{和}\qquad{}^*J_{abc}=\varepsilon_{abcd}J^d,$$

这里，按[§14.7](chapter_14.md#147-度规能为你做什么)，$F_{ab}$ 的指标升 $F^{ab}$ 就是 $g^{ac}g^{bd}F_{cd}$。注意到 $\varepsilon^{abcd}=g^{ap}g^{bq}g^{cr}g^{ds}\varepsilon_{pqrs}$ 满足 $\varepsilon^{0123}=-1$，因此[§12.7](chapter_12.md#127-体积元求和规则)里的 $\mathbf{\varepsilon}$ 由 $\mathbf{\varepsilon}_{abcd}=-\mathbf{\varepsilon}^{abcd}$ 给定。*[19.1] [图19.1](assets/page340_fig01.jpg)以图表方式给出了这些“对偶”运算（以及麦克斯韦方程本身）。我们将发现，这种意义（以及其他相关意义）下的“对偶”在各种不同
...
我们还必须对霍奇对偶的几何意义作些陈述。从 [§12.7](chapter_12.md#127-体积元求和规则) 我们知道，双矢量 $\mathbf{H}$（由反对称量 $H^{ab}$ 描述）到其"对偶"2 形式 $\mathbf{H}^\#$（由 $\frac{1}{2}\varepsilon_{abcd}H^{cd}$ 给出）的运算不会对其几何解释造成太大影响。例如，如果 $\mathbf{H}$ 是简单双矢量，从而 2 形式 $\mathbf{H}^\#$ 也是简单的（见 [§12.7](chapter_12.md#127-体积元求和规则) 节末），那么，由 $\mathbf{H}^\#$ 确定的二维平面元与由 $\mathbf{H}$ 确定的二维平面元将完全相同（正如 [§12.7](chapter_12.md#127-体积元求和规则) 指出的，唯一差别就是 $\mathbf{H}^\#$ 具有密度性质）。另一方面，从 2 形式 $H_{ab}$ 到双矢量 $H^{ab}$（$=H_{cd}g^{ca}g^{db}$）的指标升运算则更具重要的几何作用。对于简单双矢量情形，由 $H_{ab}$ 确定的二维平面元是由 $H^{ab}$ 确定的二维平面元的正交补（见 [§18.3](chapter_18.md#183-洛伦兹正交性时钟悖论)）。霍奇对偶，正如它作用到 2 形式 $H_{ab}$ 上将我们带到 $\frac{1}{2}\varepsilon_{abcd}H^{cd}$（即 $\mathbf{H}^\#$）一样，使用指标升 $H_{ab}\mapsto H^{ab}$ 并因此牵涉到正交补的传递，见[图 19.2](assets/page341_fig01.jpg)。相应地，将我们从 $\mathbf{F}$ 带到 $^*\mathbf{F}$ 的霍奇对偶也涉及正交补。

有了这种记法，我们就可以非常简洁地将麦克斯韦方程写成**(19.2)**

$$\mathrm{d}\mathbf{F}=0,\quad \mathrm{d}\,^*\mathbf{F}=4\pi\,^*\mathbf{J}.$$
...
**图 19.2** 在四维空间里，简单双矢量 $\mathbf{H}$（$H^{ab}$）及其"对偶"2形式 $\mathbf{H}^\#\left(\frac{1}{2}\varepsilon_{abcd}H^{cd}\right)$表示的是同一个二维平面元。但 $\mathbf{H}$ 的指标降（即简单2形式 $H_{ab}$，它等价于其"对偶"双矢量 $\frac{1}{2}\varepsilon^{abcd}H_{cd}$）表示的则是二维正交补平面元（见图 18.4）。因此，正是霍奇对偶中的指标升/降导致了到正交补的通道。

我们还可以将麦克斯韦方程完全写成指标形式\*[19.3]

$$\nabla_{[a}F_{bc]}=0\quad,\quad \nabla_a F^{ab}=4\pi J^b。$$

注意，如果我们将外导数算子 $\mathbf{d}$ 用到麦克斯韦方程第二式 $\mathbf{d}\,{}^\ast\!\mathbf{F}=4\pi\,{}^\ast\!\mathbf{J}$ 的两边，并利用 $\mathbf{d}^2=0$ 这一事实（[§12.6](chapter_12.md#126-外导数)），则可得推论：电荷-电流矢量 $\mathbf{J}$ 满足"零散度"方程\*\*[19.4]

$$\mathbf{d}\,{}^\ast\!\mathbf{J}=0\qquad\text{或等价地}\qquad \nabla_a J^a=0。$$
...
$${}^+\!\mathbf{F}=\frac{1}{2}(\mathbf{F}-\mathrm{i}\,{}^\ast\!\mathbf{F})\quad\text{和}\quad {}^-\!\mathbf{F}=\frac{1}{2}(\mathbf{F}+\mathrm{i}\,{}^\ast\!\mathbf{F})$$
...
$${}^\ast({}^\pm\!\mathbf{F})=\pm\,\mathrm{i}\,{}^\pm\!\mathbf{F}$$

记住，${}^\ast\!\mathbf{J}$ 是实的，我们可以将两组麦克斯韦方程（分别按实部和虚部）叠加成一组：

$$\mathbf{d}\,{}^+\!\mathbf{F}=-2\pi\mathrm{i}\,{}^\ast\!\mathbf{J}$$
...
??? question "答案 [19.3]"
    麦克斯韦方程可写成外微分形式 $dF=0$ 与 $d{*F}=4\pi {*J}$。第一个方程等价于 $F$ 来自势 $A$ 的外导数，即无磁单极和法拉第感应定律；第二个方程给出高斯定律和安培-麦克斯韦定律。

    把 $F_{ab}$ 分解成电场和磁场分量，再把四维散度按时间分量和空间分量拆开，就逐项得到通常三维矢量形式的麦克斯韦方程。

\*\* [19.4] 证明这两种零散度形式是等价的。

??? question "答案 [19.4]"
    对偶张量定义为 ${*F}_{ab}=\frac12\varepsilon_{ab}{}^{cd}F_{cd}$。方程 $dF=0$ 的指标形式是 $\nabla_{[a}F_{bc]}=0$，把它与体积形式收缩，就等价于 $\nabla^a {*F}_{ab}=0$。

    反过来，再用体积形式收缩 $\nabla^a {*F}_{ab}=0$ 可恢复完全反对称的三指标方程。因此“闭形式”和“对偶的零散度”只是同一组方程的两种写法。

\*\*\* [19.5] 证明该式。首先说明，一个变量对偶两次产生负的原变量。这个负号与时空的洛伦兹符号差有关吗？请予以解释。

??? question "答案 [19.5]"
    在四维洛伦兹符号差下，对二形式取两次霍奇对偶给出 ${**F}=-F$。这个负号来自一般公式 ${**}=(-1)^{p(n-p)+s}$；这里 $n=4$、$p=2$，而洛伦兹度规有一个时间号差，所以得到负号。

    因此 $F\pm i{*F}$ 在再取对偶时分别只乘以 $\mp i$，成为复的自对偶和反自对偶部分。若是欧氏四维符号差，则二形式上 ${**}=+1$，相应分解不需要这个 $i$。
...
## 19.3　麦克斯韦理论中的守恒律和通量定律 { .tutor-section }

我们由电荷－电流矢量的零散度可得到电荷守恒方程。因为通过外演算基本定理（[§12.6](chapter_12.md#126-外导数)），我们有$\int_{\mathcal{R}} \mathrm{d} {}^{*}\mathbf{J} = \int_{\partial\mathcal{R}} {}^{*}\mathbf{J}$，因此

$$\int_{\mathcal{Q}} {}^{*}\mathbf{J} = 0,$$

积分在闵可夫斯基空间M的任意三维闭曲面$\mathcal{Q}$上进行。（M的任意三维闭曲面是M的某个四维紧致区域$\mathcal{R}$的边界$\partial\mathcal{R}$。）见[图19.3](assets/page343_fig01.jpg)。量${}^{*}\mathbf{J}$可理解为穿过$\mathcal{Q} = \partial\mathcal{R}$的"电荷通量"（或电荷"流量"）。因此，上述方程告诉我们的是，穿过边界的净电荷通量为零；即所有流入$\mathcal{R}$的电荷量精确等于所有从$\mathcal{R}$流出的电荷量：电荷守恒。***[19.7]

我们还可以从麦克斯韦方程第二式$\mathrm{d} {}^{*}\mathbf{F} = 4\pi {}^{*}\mathbf{J}$导出所谓"高斯定理"。在某一时刻$t = t_0$应用这一定理，就可得到三维的外演算基本定理。它告诉我们，$t_0$时刻处于二维闭曲面$S$内的总电荷值（见[图19.4](assets/page343_fig02.jpg)），可以表为麦克斯韦张量${}^{*}\mathbf{F}$的对偶在$S$上的积分。或者说，通过积分穿过$S$的电场$\mathbf{E}$的总通量，我们可以获得$S$包围的总电荷量。***[19.8]

更一般地，即使$S$不处于某个固定时间点$t = t_0$，这个定理一样可用。假定$S$是某个三维紧致空间区域$\mathcal{A}$的二维类空边界。于是$S$所包围的（或用时空术语说，就是"穿过"$S$的，见[图19.4](assets/page343_fig02.jpg)）区域$\mathcal{A}$内的总电荷量$\chi$为

$$\int_{S} {}^{*}\mathbf{F} = 4\pi\chi,\ \text{这里}\ \ \chi = \int_{\mathcal{A}} {}^{*}\mathbf{J}.$$
...
从麦克斯韦方程第一式 $\mathrm{d}\mathbf{F}=0$ 我们也可以得到相关的守恒律。除了 $\mathbf{F}$ 取代了 ${}^{*}\mathbf{F}$，相应的源项 ${}^{*}\mathbf{J}$ 为零，这个式子与麦克斯韦方程第二式别无二致。因此，对闵可夫斯基空间内任意二维闭曲面，[^2] 我们总有通量定律

$$\int_{\mathcal{S}} \mathbf{F} = 0 \text{。}$$

注意，从 ${}^{*}\mathbf{F}$ 变换到 $\mathbf{F}$（或从 $\mathbf{F}$ 到 ${}^{*}\mathbf{F}$），我们只是交换了电场和磁场矢量（符号也作相应变化）。$\mathbf{F}$ 的无源性表达了这么一个（迄今所知的）事实：自然界不存在磁单极子。一个磁单极子本身就是一个磁北极或磁南极——而不是像普通磁体那样南北极总是成对出现。（普通的磁极不是一个独立的物理实体，只是电荷环流的一种表现。）自然界里，物理客体上似乎不存在净"磁荷"（非零"磁力"）。单从麦克斯韦方程的观点上看，似乎找不到好的理由来说明为什么不存在磁单极子，因为我们可以简单地为麦克斯韦方程第一式 $\mathrm{d}\mathbf{F}=0$ 的右边配备一个量，这并不造成协调性的破坏。事实上，物理学家们一直在思考磁单极子存在的可能性，并试图找到它们。它们的存在对粒子物理有着重要影响（见 [§28.2](chapter_28.md#282-宇宙的拓扑缺陷)），但直到目前还没有任何迹象表明这种单极子在宇宙中实际存在着。

## 19.4 作为规范曲率的麦克斯韦场 { .tutor-section }

对某个 1 形式 $\mathbf{A}$，麦克斯韦方程第一式 $\mathrm{d}\mathbf{F}=0$ 还隐含着

$$\mathbf{F} = 2\mathrm{d}\mathbf{A} \text{，}$$

（这里用到了"庞加莱引理"：如果 $r$ 形式 $\mathbf{\alpha}$ 满足 $\mathrm{d}\mathbf{\alpha}=0$，那么局域上总存在一个 $(r-1)$ 形式 $\mathbf{\beta}$ 使得 $\mathbf{\alpha}=\mathrm{d}\mathbf{\beta}$。见 [§12.6](chapter_12.md#126-外导数)。）此外，在一个具有欧几里得拓扑的区域内，这个局域结果可扩展到整个区域。[^3] 量 $\mathbf{A}$ 称为电磁势。它并不由场 $\mathbf{F}$ 唯一地确定，但可以有一个增量 $\mathrm{d}\Theta$，\*[19.9] 这里 $\Theta$ 是某个实标量场：

$$\mathbf{A} \mapsto \mathbf{A} + \mathrm{d}\Theta$$
...
电磁势的这个"规范自由度"告诉我们，$\mathbf{A}$ 不是一个局域可测的量。我们不可能从实验上测得"$\mathbf{A}$ 在某点的值"，因为 $\mathbf{A}+\mathrm{d}\Theta$ 可以像 $\mathbf{A}$ 一样满足同样的物理要求。但这个势为处理麦克斯韦场与某种其他物理实体 $\Psi$ 之间的相互作用提供了数学上的线索。它是怎么起作用的呢？$A_a$
...
他的基本方法是将电磁势编译到丛联络里，就像我前面做的那样，但 $\nabla_a$ 的表达式里不含虚单位“i”。我们可将 $\mathcal{M}$ 上有关的丛看成是由那些相同的零锥共同所具有的洛伦兹度规给定的。这样，$\mathcal{M}$ 的某个点 $x$ 上的纤维组成一族成正比的度规族（如果愿意，我们可取比例系数为正）。这些度规就是上述 $g \mapsto \lambda g$ 里可能的“$\lambda$”。对任一特定选取的度规，我们有一种沿曲线定义的距离和时间规范。但规范的选取不是绝对的，因此不存在从成比例的度规 $g$ 等价族中进行优选的问题。除了零锥的（共形）结构之外还存在某种结构，即丛联络——或叫规范联络——它是外尔为使麦克斯韦的 $\mathbf{F}$（即 $F_{ab}$）成为其曲率而引入的。这种曲率测度的是如[图 19.5](assets/page346_fig01.jpg)（a）所示的（当世界线出现如[图 19.5](assets/page346_fig01.jpg)（b）的无穷小偏离时的）时钟快慢上的偏差。（这一点也可与 [§15.8](chapter_15.md#158-丛曲率) 的[图 15.16](assets/page270_fig01.jpg) 和[图 15.19](assets/page273_fig01.jpg) 里 $\mathbb{C}$ 上的“张紧的丛” $\mathcal{BC}$ 进行比较，基本的丛概念非常类似。）
...
有意思的是，联络的路径依赖性（我们可以比较一下[图19.5](assets/page346_fig01.jpg)里的路径依赖性）表现为一种出人意表的实验现象，即所谓阿哈罗诺夫-玻姆（Aharonov-Bohm）效应。由于联络 $\mathbf{\nabla}$ 的作用只是一种量子现象，因此我们在经典实验上是看不到这种路径依赖性的。阿哈罗诺夫-玻姆效应有赖于量子干涉现象（见 [§21.4](chapter_21.md#214-量子理论的实验背景) 和[图21.4](assets/page384_fig01.jpg)）。在其典型实验里，电子被准直成束后由分束片分成两束通过一个无电磁场（$\mathbf{F}=0$）的区域，区域中两束电子路径之间置有一长直密绕螺管（管内充满磁力线，管外无磁场），最后两束电子在检测屏上形成干涉条纹（见[图19.6](assets/page347_fig01.jpg)(a)）。整个过程中电子都处于无场 $\mathbf{F}$ 作用状态。但相关的无场区 $\mathcal{R}$（自源始，分束绕过密绕螺管，直至屏）却不是单连通的，对于 $\mathcal{R}$ 外的场 $\mathbf{F}$，不存在一种规范选择使得在 $\mathcal{R}$ 内电磁势 $\mathbf{A}$ 处处为零。正是这种非单连通 $\mathcal{R}$ 内的非零势的存在——或更准确地说，$\mathcal{R}$ 内 $\mathbf{\nabla}$ 的路径依赖性——导致屏上干涉条纹出现位移。
...
**图19.6** 阿哈罗诺夫-玻姆效应。(a) 一束电子被分成两束绕过导电长直密绕螺管的两边，最后在检测屏上形成量子干涉条纹（比较图21.4）。尽管电子经过的是一个零场强（$\mathbf{F}=0$）区域，但干涉条纹却与螺管内的磁通量强度有关。(b) 这个效应依赖于 $\oint\!A$ 的值，这个值在积分路径是拓扑上非平凡闭合曲线时不为零，尽管路径上的 $\mathbf{F}$ 是零。在无场区无论怎么连续地改变路径，$\oint\!A$ 的值都不会变化。

事实上，干涉条纹位移效应并不依赖于 $\mathbf{A}$ 在局部的具体值（这也不可能，因为如上所述，$\mathbf{A}$ 不是一个局部可观测量），而是取决于 $\mathbf{A}$ 的某个非局域积分。这就是量 $\oint\!A$，环路取为 $\mathcal{R}$ 内的拓扑非平凡闭曲线。见[图19.6](assets/page347_fig01.jpg)(b)。由于 $\mathcal{R}$ 内 $\mathrm{d}A$ 为零（因为 $\mathcal{R}$ 内 $\mathbf{F}=0$），因此如果在 $\mathcal{R}$ 内沿
...
??? question "答案 [19.11]"
    规范变换把波函数乘以局域相位 $e^{ie\chi}$，同时把势改为 $A_a+\partial_a\chi$。这样协变导数作用后的结果也只乘以同一个相位，物理态不变。

    曲率 $F_{ab}=\partial_aA_b-\partial_bA_a$ 中二阶偏导数相互抵消，所以不受 $A_a\mapsto A_a+\partial_a\chi$ 影响。这就是为什么可观测电磁场不是势本身，而是势的曲率。
...
以从一个速度变换到另一个速度，并得到质心运动的守恒，见 §§ 18.6，7，[图 18.16](assets/page332_fig01.jpg)。）为了从任一具体的基灵矢量 $\mathbf{\kappa}$ 得到适当的守恒律，我们构造通量

$$L_a = T_{ab}\kappa^b,$$

只要对称张量 $T_{ab}$ 满足 $\nabla^a T_{ab}=0$，就一定有守恒律 $\nabla_a L^a = 0$。***[19.14] 因此，如同 [§19.3](chapter_19.md#193-麦克斯韦理论中的守恒律和通量定律) 情形，存在积分守恒律 $\int_{\mathcal{Q}} {}^*L = 0$。

这些守恒律只在具有适当的、由基灵矢量 $\mathbf{\kappa}$ 给出的对称性的时空里才成立。物理上看，其原因在于时空几何的自由度——即引力——受到场的去耦。时空几何仅仅是一个背景，因此不受其中的场的干扰。此外，由于对称性，场不能够从背景中得到前面所讨论的那个量（或将这个量遗留在背景里）。这些考虑对以后的讨论（特别是第 30 章里的 §§ 30.6，7）非常重要，但它们毕竟无助于我们了解守恒律在出现了引力情形下的命运，此时我们仍无法找回失去了的能量和动量守恒律。

自广义相对论出现以后，这一令人难堪的事实一直引起人们对这一理论的强烈质疑，多年来许多物理学家都表达过这种不快。⁶ 以后（[§19.8](chapter_19.md#198-引力场能量)）我们会看到，实际上爱因斯坦理论是以一种相当圆滑的方式来考虑能量动量守恒的——至少在这种守恒律必不可少的情形下是如此。现在，我们就来谈谈这一点。在爱因斯坦理论里，出现在场方程里的对称 $\begin{bmatrix} 0 \\ 2 \end{bmatrix}$ 价张量 $\mathbf{T}$ 包含了所有非引力场（和粒子）的能量动量。而引力场本身的能量则排除在 $\mathbf{T}$ 的表示之外。

如果我们再次考虑到等效原理，就会明白这种观点是合理的。想像一个处于自由轨道上的观察者，譬如说处于没有窗户的飞船中，于是乎，至少从一级近似上看，根本就不存在引力场。观察者将看到，在飞船内能量是守恒的，因此方程 $\nabla^a T_{ab}=0$ 里不含任何引力场的贡献。当然这种"守恒"只是一种近似，只要存在由引力场的不均匀性（如 [§17.5](chapter_17.md#175-嘉当的牛顿时空) 中的[图 17.8](assets/page305_fig01.jpg)(a), 17.8(b) 和 17.9）引起的相对加速度（潮汐）效应，就必须给予修正。于是问题变得微妙起来，我们得确认在不同情形下哪一"级"效应开始起作用。但总的来说，其结果是：量 $\mathbf{T}$ 及其方程 $\nabla^a T_{ab}=0$ 应当在不受引力场非均匀性的干扰下——即不受时空联络 $\mathbf{\nabla}$ 的曲率 $\mathcal{R}$ 的影响下——得到保留，引力对能量动量守恒的影响应当作为某种非局域效应在总能量动量计算中进行修正。（唯一的例外可能是我们如何来考虑时空曲率带来的修正，但这个问题在这里并不重要。）从这个观点来看，引力对能量动量的影响在一定意义上——将局域方程 $\nabla^a T_{ab}=0$ 从总体能量动量的积分守恒律分离开来——"被遗漏了"。
...
??? question "答案 [19.16]"
    李导数刻画沿矢量场拖动张量后的变化。对联络或曲率使用李导数时，可把它展开为协变导数项加上曲率项；这正是里奇恒等式给出的二阶协变导数不对易的结果。

    将这些项代入正文的表达式，含二阶导数的部分按对称化相互抵消，剩下的曲率项就是所需公式。图示记法中的线移动对应的正是这一步的里奇恒等式。

\**\**\** [19.17]* 充分说明为什么我们可以"去掉"所有的 $t^a$？解释张量对称性的作用。

??? question "答案 [19.17]"
    等式对任意矢量场 $t^a$ 都成立，而 $t^a$ 在一点的取值可任意指定。因此若一个张量与所有 $t^a$ 收缩都为零，则该张量本身必须为零。

    这里还要用到张量的对称性：某些反对称部分与对称组合收缩时自动消失，剩下的独立分量由任意 $t^a$ 完全探测。因此可以“去掉”所有 $t^a$，得到纯张量等式。

\**\**\** [19.18]* 用图示记法证明这一点。

??? question "答案 [19.18]"
    图示证明的核心是把每个曲率项画成同一个线图的不同连线方式。利用第一比安基恒等式，三种循环排列的曲率图之和为零。

    再用度规收缩把相邻线并合，图中可滑动的线对应协变导数与度规相容。整理后，剩下的线图正是正文所写的张量恒等式。
...
## 19.7 进一步的问题：宇宙学常数；外尔张量 { .tutor-section }
...
??? question "答案 [19.20]"
    牛顿极限中只有弱场的 $g_{00}$ 分量显著，它满足类似泊松方程的关系。若取爱因斯坦方程左边的迹修正组合，时间时间分量在低速物质中不仅接收 $T_{00}$，还受到取迹项的贡献。
...
??? question "答案 [19.23]"
    外尔张量是从黎曼曲率中去掉所有由里奇张量和标量曲率决定的迹部分后剩下的部分。它的定义正是选择这些减去项，使任意一对可缩并指标的收缩都抵消。

    例如收缩 $C_{abc}{}^d$ 时，黎曼张量的迹给出里奇张量，而后面减去的 $g$ 与 $R_{ab}$、$R$ 项给出完全相同的迹贡献，符号相反。图示记法中这表现为所有带闭合收缩线的部分被显式扣除，因此外尔张量是无迹的。
...
## 19.8　引力场能量 { .tutor-section }

我们回到引力场本身的质量/能量问题上来。虽然在诸如能量动量张量 $T$ 的问题上没有回旋余地，但显然还存在这样一种情形，就是“无形的”引力能量实际所起的物理角色。想象两个大质量物体（譬如行星）。如果它们靠得很近（我们可以假定它们同时处于相互间的静态），那么总能量里就会有（负的）引力势能的贡献，因此总质量较二者远离时为小（[图19.8](assets/page355_fig01.jpg)）。忽略更小的能量效应（如星体表面因另一个星体的引潮场作用而产生的变形），我们看到，不论两个星体之间是靠近还是远离，实际能量动量张量 $T$ 的总贡献都是一样的。但总质量/能量在两种情形下则不同，这种差异可能起因于引力场本身的能量（实际上是负贡献，二者接近的情形较远离的情形作用更大）。
...
介于这两个极端之间的中间情形是著名的双中子星系统 PSR1913+16 发出的引力波，由约瑟夫·泰勒（Joseph Taylor）和罗素·赫尔斯（Russell Hulse）领导的诺贝尔获奖小组曾对此展开研究，见[图19.9](assets/page356_fig01.jpg)。（中子星是一种高致密星体，主要由中子组成，整个星体的密度堪比原子核。一颗由这种物质构成的网球其质量差不多相当于火卫二的质量！）人们对这个系统已观察了
