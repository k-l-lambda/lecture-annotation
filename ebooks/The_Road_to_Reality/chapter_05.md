...
??? question "答案 [5.1]"
    加法的退化情形包括 $w=0$、$z=0$ 或 $w,z$ 共线时，平行四边形压成线段；若 $w=-z$，则 $w+z=0$，四个顶点中有重合。所有这些仍符合向量相加的平移规则。

    乘法的退化情形主要是 $w=0$ 或 $z=0$，这时相似三角形压缩到原点，结果 $wz=0$；若 $w=1$ 或 $z=1$，相应三角形不变；若 $w$ 或 $z$ 是负实数，相似关系表现为转过 $\pi$ 再按模长伸缩。这些都与复数乘法公式一致。

\*\*〔5.2〕 不妨试试。

??? question "答案 [5.2]"
    写 $w=a+ib$，$z=c+id$。加法给出 $w+z=(a+c)+i(b+d)$，这正是平面中从 $0$ 先走到 $w$ 再平移 $z$ 的平行四边形终点。

    对乘法，$wz=(ac-bd)+i(ad+bc)$。若 $z$ 的坐标向量为 $(c,d)$，乘以 $w$ 后变为 $(ac-bd,ad+bc)$。这个线性变换把 $(1,0)$ 送到 $(a,b)$，把 $(0,1)$ 送到 $(-b,a)$，两像向量等长且垂直，因此它是一个伸缩加旋转，伸缩因子为 $|w|$。这正是相似三角形法则。
...
（见[§2.1](chapter_02.md#21-毕达哥拉斯定理)）——将原点0移至点w；见[图5.2](assets/page078_fig03.jpg)(a)。平行四边形法则就是对这种情形的复数。但什么是乘法映射呢？它是一种保持原点和形状不变的变换——将点1变换到点w。在一般情形下，它包括具有均匀扩展（或收缩）的（非反射）转动变换，见[图5.2](assets/page078_fig03.jpg)(b)。***[5.3]相似三角形法则有效地表明了这一点。这个映射对于我们在[§8.2](chapter_08.md#82-共形映射)的讨论有重要意义。
...
??? question "答案 [5.3]"
    映射 $z\mapsto wz$ 保持加法：两点差向量 $z_1-z_2$ 被送到 $w(z_1-z_2)$，所以直线、平行性和平行四边形结构都保持。它还满足 $w(iz)=i(wz)$，即先把向量转直角再映射，等于先映射再转直角。

    因此一个直角坐标框架在此映射下仍变成直角坐标框架，只是所有单位长度都被同一个因子 $|w|$ 放大，并整体旋转到 $1\mapsto w$ 的方向。于是该映射必为绕原点的旋转加均匀伸缩。
...
在复数的平面描述中，我们对平面上的点用的是标准的笛卡儿坐标$(x,y)$，但我们也可以用极坐标$[r,\theta]$。这里正实数$r$度量自原点始的距离，角度$\theta$度量直线自原点到点$z$的实轴按逆时针方向转过的角度，见[图5.4](assets/page079_fig02.jpg)(a)。量$r$也叫作复数$z$的模，我们经常写作

$$r = |z|,$$

$\theta$叫做幅角（在量子力学里，有时也称作相角）。对$z=0$点，我们不考虑其幅角$\theta$，但仍定义$r$为自原点的距离，此时就是$r=0$。

为清楚起见，我们可要求$\theta$的主值处于一定的象限范围内，例如$-\pi < \theta \leqq \pi$（这是标准约定）。同时我们也可以认为对幅角增加$2\pi$的整数倍而其效果不变。这使我们能够在度量角度时可以不论正反方向绕原点转任意圈（见[图5.4](assets/page079_fig02.jpg)(b)）。（这第二种观点实际上是一种更深刻的观点，它的意义我们不久就会知道。）由[图5.5](assets/page079_fig03.jpg)和基本三角函数关系我们看到，
...
??? question "答案 [5.4]"
    令 $w=r(\cos\theta+i\sin\theta)$，$z=s(\cos\phi+i\sin\phi)$。相乘得 $wz=rs\{(\cos\theta\cos\phi-\sin\theta\sin\phi)+i(\sin\theta\cos\phi+\cos\theta\sin\phi)\}$。

    括号内由三角加法公式就是 $\cos(\theta+\phi)+i\sin(\theta+\phi)$。所以模相乘为 $rs$，幅角相加为 $\theta+\phi$。
...
??? question "答案 [5.5]"
    将两个级数相乘：$e^a e^b=(\sum_{p\ge0}a^p/p!)(\sum_{q\ge0}b^q/q!)$。其中总次数为 $n=p+q$ 的项之和为 $\sum_{p+q=n}a^p b^q/(p!q!)$。

    而 $e^{a+b}=\sum_{n\ge0}(a+b)^n/n!$。由二项式定理，$(a+b)^n/n!=\sum_{p+q=n}a^p b^q/(p!q!)$。逐次比较各总次数的项，得到 $e^{a+b}=e^a e^b$。
...
这里 $\log r$ 是正实数 $r$ 的普通的自然对数——实指数的反函数。为什么呢？这从[图 5.7](assets/page083_fig01.jpg) 看得很清楚，这种实对数是存在的。在[图 5.7](assets/page083_fig01.jpg)(a) 中，我们画出了 $r=e^x$ 的图像。只需将坐标轴颠倒个个儿，我们即得到反函数 $x=\log r$ 的图像如[图 5.7](assets/page083_fig01.jpg)(b)。毫不奇怪，$z=\log w$ 的实部正是普通的实对数。奇怪的倒是<sup>4</sup> $z$ 的虚部恰好就是复数 $w$ 的幅角 $\theta$。这一事实证实了我们早先所说的复数的幅角实际就是一种对数形式的断言。

![图 5.7 (a) 函数 $r=e^x$ 的图像；(b) 反函数 $x=\log r$ 的图像。](assets/page083_fig01.jpg)

图 5.7 为了得到正实数 $r$ 的对数，考虑图 (a) $r=e^x$ 的图像。这个图像囊括了 $r$ 的所有正值，因此，将这幅图像颠倒一下，我们就得到了关于正值 $r$ 的反函数 $x=\log r$ 的图像 (b)。

此前我们说过，复数的幅角定义存在不确定性。我们可以让 $\theta$ 加上 $2\pi$ 的任意整数倍而实际效果不变（回忆[图 5.4](assets/page079_fig02.jpg)(b)）。相应地，在 $w=e^z$ 中，对给定的 $w$ 存在多种不同的解 $z$。如果我们取定一个这样的 $z$，则 $z+2\pi i n$ 也是可能的解，其中 $n$ 是任意整数。因此，$w$ 的对数只能确定到相差一个任意倍 $2\pi i$ 的程度。必须记住，在诸如 $\log ab=\log a+\log b$ 的表达式里也存在这种不确定性，我们必须对对数做出适当的选择。
...
??? question "答案 [5.6]"
    若 $z$ 是 $w$ 的对数，则 $e^z=w$。又由欧拉公式 $e^{\pi i}=-1$，所以 $e^{z+\pi i}=e^z e^{\pi i}=w(-1)=-w$。

    因此 $z+\pi i$ 的指数正是 $-w$，按定义它就是 $-w$ 的一个对数。
...
从这种方式我们看得很明白，如果两个复数相乘，得到的是它们的模的积和幅角的和（$re^{i\theta}se^{i\phi} = rse^{i(\theta+\phi)}$，故 $r$ 和 $s$ 相乘，而 $\theta$ 和 $\phi$ 相加——记住，从 $\theta + \phi$ 上减去 $2\pi$ 不会造成任何影响），就像[图 5.1](assets/page078_fig01.jpg)b 的相似三角形法则所显示的那样。以后我不再讨论记号 $[r, \theta]$，而是直接用上述表达式。注意，若 $r=1$，$\theta=\pi$，则我们得到 $-1$ 并回到[图 5.4](assets/page079_fig02.jpg)(a) 几何所示的欧拉定理 $e^{\pi i} + 1 = 0$；若 $r=1$，$\theta=2\pi$，则我们得到 $+1$ 和 $e^{2\pi i} = 1$。
...
??? question "答案 [5.7]"
    由 $e^{i(a+b)}=e^{ia}e^{ib}$，左边为 $\cos(a+b)+i\sin(a+b)$。右边为 $(\cos a+i\sin a)(\cos b+i\sin b)$。

    展开右边得 $(\cos a\cos b-\sin a\sin b)+i(\sin a\cos b+\cos a\sin b)$。比较实部和虚部，就得到正文两条加法公式。
* [5.8] 验证一下。

??? question "答案 [5.8]"
    由 $e^{3i\theta}=(e^{i\theta})^3$，有 $\cos3\theta+i\sin3\theta=(\cos\theta+i\sin\theta)^3$。

    展开右边：实部为 $\cos^3\theta-3\cos\theta\sin^2\theta$，虚部为 $3\sin\theta\cos^2\theta-\sin^3\theta$。比较实部和虚部即得正文公式。
...
??? question "答案 [5.9]"
    写 $w=Re^{i\Theta}$，$z=a+ib$。则 $\log w=\log R+i(\Theta+2\pi n)$，其中 $n\in\mathbb Z$。于是 $w^z$ 的所有值为 $e^{(a+ib)(\log R+i(\Theta+2\pi n))}$。

    其模的对数为 $a\log R-b(\Theta+2\pi n)$，幅角为 $b\log R+a(\Theta+2\pi n)$。当 $n$ 变化时，点的对数模和幅角都按等差数列变化，所以这些点落在对数螺线上；从正、负方向追踪可看作两族等角螺线的交点。若 $z$ 为有理实数，只得到有限多个不同值；若 $z$ 为无理实数或有非零虚部，通常得到无限多个不同值。

** [5.10] 解这个“疑难”：因为 $e = e^{1+2\pi i}$，故 $e = (e^{1+2\pi i})^{1+2\pi i} = e^{1+4\pi i - 4\pi^2} = e^{1-4\pi^2}$。

??? question "答案 [5.10]"
    错误在于把幂运算当成单值且无条件满足 $(e^u)^v=e^{uv}$。表达式 $(e^{1+2\pi i})^{1+2\pi i}$ 要先为底数 $e^{1+2\pi i}=e$ 选定一个对数。

    若按标准约定取 $\log e=1$，则结果是 $e^{1+2\pi i}=e$。若故意取 $\log e=1+2\pi i$，则得到 $e^{(1+2\pi i)^2}=e^{1-4\pi^2}$。这不是矛盾，而是同一多值幂的另一个分支值。

* [5.11] 证明这一点。

??? question "答案 [5.11]"
    若规定了 $\log b$，则 $b^z=e^{z\log b}$。要求 $b^z=w$ 等价于 $e^{z\log b}=w$，所以 $z\log b$ 必为 $\log w$ 的某个值。

    因而 $\log_b w=(\log w)/(\log b)$。由于 $\log w$ 可加上 $2\pi i n$，$\log_b w$ 就可加上 $2\pi i n/\log b$。这就是正文所说的不确定性。

* [5.12] 为什么这是一种容许的规定？

??? question "答案 [5.12]"
    因为 $i$ 的模为 $1$，幅角可取 $\pi/2$。按复对数公式，$\log i=\log1+i(\pi/2)=\pi i/2$。

    当然还可以取 $\pi i/2+2\pi i n$。正文只是选取其中的主值分支，因此是完全容许的规定。

* [5.13] 证明为什么这是有效的。

??? question "答案 [5.13]"
    $\log i$ 的所有值为 $\pi i/2+2\pi i n$。于是 $i^i=e^{i\log i}=e^{i(\pi i/2+2\pi i n)}=e^{-\pi/2-2\pi n}$。

    这等于主值 $e^{-\pi/2}$ 乘以 $e^{-2\pi n}$。把整数 $n$ 改名为 $-n$，也可说通过乘以 $e^{2\pi n}$ 得到所有值。它们全是正实数。
...
??? question "答案 [5.14]"
    令 $\log w=L+2\pi i k$。则 $w^{1/n}$ 的候选值为 $e^{(L+2\pi i k)/n}$。当 $k=0,1,\ldots,n-1$ 时得到 $n$ 个不同值。

    若把 $k$ 增加 $n$，值会乘以 $e^{2\pi i}=1$，所以不会产生新值。并且每个候选值的 $n$ 次方都是 $e^{L+2\pi i k}=w$。因此正好得到 $z^n=w$ 的全部 $n$ 个根。

**[5.15] 证明这一点。

??? question "答案 [5.15]"
    先固定 $\log w=L$，定义 $w^a=e^{aL}$。若随后把 $\log(w^a)$ 规定为 $aL$，则 $(w^a)^b=e^{b\log(w^a)}=e^{baL}=w^{ab}$。

    如果不作这个规定，而任意选择 $\log(w^a)$ 的其他分支，就可能多出 $2\pi i$ 的整数倍并改变结果。这正是正文强调必须明确对数选择的原因。
