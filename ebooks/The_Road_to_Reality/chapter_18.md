...
具有更直接的物理意义，因为它沿作为允许的有质量粒子世界线的类时曲线为正，积分 $\int ds$（$ds > 0$）可直接理解为理想化时钟沿世界线测得的实际的物理时间。在选择（伪）度规 $\mathbf{g}$（其指标形式为 $g_{ab}$）时，我将用（＋－－－）符号差定义，以便上述表达式可写成指标形式（[§13.8](chapter_13.md#138-正交群)）
...
与采用这种看起来不自然的处理（至少我认为是这样）方式不同，我们来试着“走极端”，看看将所有坐标都取为复数（见[图18.2](assets/page318_fig01.jpg)）会怎样。这时不存在符号差上的区别，复坐标ω，ξ，η，ζ现在表示的是复空间 ℂ⁴，我们可以将它视为 𝔼⁴ 的复化空间 ℂ𝔼⁴。作为复仿射空间（[§14.1](chapter_14.md#141-流形上的微分)），它与 𝕄 的复化 ℂ𝕄 一样。此外，ℂ𝔼⁴ 和 ℂ𝕄 的每个复四维空间有一个完全等价的平直（曲率为零）复度规 ℂ**g**。这个度规可取 ds² = dω² + dξ² + dη² + dζ² 形式，这里 𝔼⁴ 是 ℂ𝕄 的实子空间，它的所有坐标 ω，ξ，η，ζ 都是实的；而 𝕄 的 ω 坐标是实的，ξ，η，ζ 都是纯虚的。作为 ω 是纯虚的但 ξ，η，ζ 都是实坐标的另一种闵可夫斯基实子空间 𝕄̃，其“ds²”有闵可夫斯基度规的“dl²”形式。𝔼⁴、𝕄 和 𝕄̃ 这三种子空间称为 ℂ𝔼⁴ 的实切片。如果我们对 ℂ𝔼⁴ 赋予一种
...
## 18.2　闵可夫斯基空间的对称群 { .tutor-section }
...
??? question "答案 [18.1]"
    $C$ 是把每个坐标做“复共轭再配一个符号”的对合，并使选定的实切片逐点不变。对 $\mathbb{E}^4$（坐标全实），$C$ 即普通复共轭 $(\omega,\xi,\eta,\zeta)\mapsto(\bar\omega,\bar\xi,\bar\eta,\bar\zeta)$。

    对 $\mathbb{M}$（$\omega$ 实，$\xi,\eta,\zeta$ 纯虚），须保持 $\omega$ 实而其余纯虚，故 $C:(\omega,\xi,\eta,\zeta)\mapsto(\bar\omega,-\bar\xi,-\bar\eta,-\bar\zeta)$；对 $\tilde{\mathbb{M}}$（$\omega$ 纯虚，$\xi,\eta,\zeta$ 实），$C:(\omega,\xi,\eta,\zeta)\mapsto(-\bar\omega,\bar\xi,\bar\eta,\bar\zeta)$。三者都满足 $C^2=1$，其固定点集恰为对应实切片，故 $\mathbb{M}$、$\tilde{\mathbb{M}}$ 情形不是标准的复共轭。

*[18.2] 你能知道为什么吗？

??? question "答案 [18.2]"
    一个实欧几里得运动按坐标写出来是一组系数为实数的（线性）代数公式，而“保度规”这个条件本身也是关于这些系数的一组多项式恒等式。把公式里的实坐标和实系数统统换成复数，公式形式完全不变，于是得到的就是 $\mathbb{CE}^4$ 的一个复运动；同样的多项式恒等式仍然成立，故它自动保住复度规 $\mathbb{C}\mathbf g$。

    反过来，保 $\mathbb{C}\mathbf g$ 的连续（即与恒等连通的）复运动由同一组多项式条件刻画，对应李代数是实李代数的复化，因此它们恰好都是这样“复化得来”的运动，群维数与实情形相同（均为 $10$）。

*[18.3] 在此情形下通过检验 4×4 李代数矩阵来确认这一点。

??? question "答案 [18.3]"
    设度规为 $\eta=\mathrm{diag}(1,-1,-1,-1)$，无穷小变换 $x^a\mapsto x^a+\epsilon X^a{}_b x^b$ 保 $\eta$ 的条件是 $\eta_{ac}X^c{}_b+\eta_{bc}X^c{}_a=0$，即矩阵 $\eta X$（下标全降）反对称。

    $4\times4$ 反对称矩阵的独立分量个数为 $\tfrac{n(n-1)}{2}=6$，与 $\eta$ 的号差无关。所以无论是 $\mathrm{O}(4)$ 还是 $\mathrm{O}(1,3)$，其李代数都是 $6$ 维，洛伦兹群确为六维。号差只改变各生成元是“真转动”还是“伪转动（boost）”，不改变维数。
...
## 18.3 洛伦兹正交性；"时钟悖论" { .tutor-section }
...
我们可以进一步贯彻这种正交性概念，在一个点 $p$ 上考虑 $r$ 维平面元 $\mathbf{\eta}$ 的正交补 $\mathbf{\eta}^\perp$。它是 $p$ 点上所有与 $p$ 点 $\mathbf{\eta}$ 的所有方向呈正交的 $(4-r)$ 维平面元 $\mathbf{\eta}^\perp$。因此，一个线元的正交补是一个三维平面元，二维平面元的正交补是另一个二维平面元，三维平面元的正交补是线元。在每一种
...
??? question "答案 [13.4]"
    庞加莱群 $=$ 四维时空平移 $\rtimes$ 洛伦兹群 $\mathrm{O}(1,3)$。它作用在“未来类时方向丛”上：丛的底空间是 $\mathbb{M}$ 的各事件点，每点的纤维是该点处指向未来零锥内部的类时方向（即可作为观察者世界线切向的方向）。

    给定两组（事件，未来类时方向），先用平移把第一个事件搬到第二个事件，再用一个保时序的洛伦兹变换（boost 加空间转动）把第一条方向转到第二条方向——boost 提供速度的自由变换，空间转动 $\mathrm{O}(3)$ 调整指向。故群在该丛上可递，这正表达了 $\mathbb{M}$ 上的相对性原理：任何点上任何惯性观察者都等价。
...
***[18.5]（i）在 $\mathbb{M}$ 内，一个三维平面元 $\mathbf{\eta}$ 在什么样的条件下才能包含其法矢量 $\mathbf{\eta}^\perp$？（ii）证明：在 $\mathbb{CE}^4$ 内，存在两族不同的二维平面，它们是自身的正交补，但二者都不可能出现在 $\mathbb{M}$ 内。（这些所谓"自对偶"和"反自对偶"二维复平面在后面具有相当的重要性，见 [§32.2](chapter_32.md#322-阿什台卡变量的手征输入) 和 [§33.11](chapter_33.md#3311-非线性引力子)。）

??? question "答案 [18.5]"
    （i）三维平面元 $\mathbf\eta$ 的法矢量 $\mathbf\eta^\perp$ 是一维方向。$\mathbf\eta$ 含 $\mathbf\eta^\perp$ 当且仅当 $\mathbf\eta^\perp$ 与 $\mathbf\eta$ 内一切方向（包括它自身）正交，即 $\mathbf\eta^\perp$ 与自身正交，亦即它是零（类光）方向。所以条件是：$\mathbf\eta$ 的法线为零方向，等价地 $\mathbf\eta$ 与光锥相切（它是“零三维平面元”）。

    （ii）在 $\mathbb{CE}^4$ 中“自正交”的二维平面就是完全迷向（totally null）的二维平面：平面内每个矢量都与平面内一切矢量正交。对号差 $(1,3)$ 的复化度规，这样的二维迷向平面恰好分成两族——自对偶与反自对偶。但在实闵可夫斯基空间 $\mathbb{M}$ 中，迷向（全零）子空间的最大维数只有 $1$（只有零方向，即沿光锥的一维方向自正交），不存在二维的全零实平面，故这两族复平面都不能落在 $\mathbb{M}$ 内。
...
## 18.4 闵可夫斯基空间的双曲几何 { .tutor-section }
...
??? question "答案 [18.6]"
    把四维闵可夫斯基空间中的二次曲面写成约束方程后，用提示中的 $x,y,w$ 作为内坐标消去 $t,z$。代回环境空间度规，交叉项抵消，得到的内禀线元就是平直欧几里得型或双曲型的标准形式，只差一个整体曲率尺度 $|\lambda|^{-1/2}$。
...
??? question "答案 [18.7]"
    双曲直线是 $\mathcal{H}^+$ 与过 $O$ 的二维平面的交。克莱因（投影）表示是从原点 $(0,0,0)$ 把 $\mathcal{H}^+$ 投影到 $t=1$ 平面；过 $O$ 的二维平面与 $t=1$ 平面相交得到的是一条直线段（弦），故双曲直线在克莱因模型里画成直线。但中心投影不保角，所以克莱因模型不是共形的。

    庞加莱表示是从“南极” $(-1,0,0)$ 作到 $t=0$ 平面的立体投影。立体投影把 $\mathcal{H}^+$ 上的“直线”映为与单位圆边界正交相交的圆弧。其共形性可由“符号差跳变”看出：立体投影在球面（实半径）情形是出了名的保角映射，把它的曲率/半径形式地换成虚的（号差跳变 $\to$ 双曲几何），映射的全纯（保角）性质保持不变，因此庞加莱模型保角，即共形——边界正交是“与无穷远共形”的标志。
...
??? question "答案 [18.8]"
    先看圆（欧氏）情形：把“速度”定义为弧所对圆心角 $\theta$ 的正切 $u=\tan\theta$。两段同方向弧长相加即角相加 $\theta_1+\theta_2$，对应速度由正切加法公式给出 $\tan(\theta_1+\theta_2)=\dfrac{\tan\theta_1+\tan\theta_2}{1-\tan\theta_1\tan\theta_2}=\dfrac{u+v}{1-uv}$。

    现在做“符号差跳变”：把圆几何换成双曲几何，圆心角 $\theta$ 换成双曲角（速率）$\rho$，正切 $\tan$ 换成双曲正切 $\tanh$，于是 $v=\tanh\rho$。双曲长度（速率）沿同一方向直接相加 $\rho_1+\rho_2$，而 $\tanh$ 加法公式分母的符号随号差跳变由 $-$ 变 $+$：$\tanh(\rho_1+\rho_2)=\dfrac{\tanh\rho_1+\tanh\rho_2}{1+\tanh\rho_1\tanh\rho_2}=\dfrac{u+v}{1+uv}$。恢复 $c$ 即得相对论速度加法 $(u+v)c/(1+uv)$。
...
??? question "答案 [18.10]"
    取参照系使 $O$ 在原点。过去光锥 $\mathcal{C}$ 的方程 $t^2=x^2+y^2+z^2$（$t<0$），其空间轨迹是随 $t$ 从过去趋于 $O$ 而半径 $|t|$ 线性收缩的球面 $S^2$。三维类时平面 $P$ 过 $O$，在该系下表现为一张随时间匀速运动的二维平面。截面 $\Sigma=\mathcal{C}\cap P$ 在每个时刻是“收缩球面”与“运动平面”的交，即一个圆，整体上这些圆扫出 $\mathcal{C}$ 上的一条（圆）轨迹。

    $O$ 点观察者沿其世界线“此刻”接收到的，是过去光锥上沿各零方向射来的光线；这些零方向与观察者瞬时空间截面相交即给出天球上的视位置。$\Sigma$ 上各点对应的零方向构成天球上的一族点，由于 $\mathcal{C}$ 与 $P$ 都是仿射（线性）对象，它们的交在天球上是一个圆——故观察者看 $\Sigma$ 是圆。换不同观察者只是换一个洛伦兹变换，而洛伦兹变换把 $(\mathcal{C},P)$ 这类线性构形仍变成同类构形，圆仍是圆。这正是无需计算就得出“天球具有共形结构、圆映为圆”的辛格论证。
...
??? question "答案 [18.14]"
    设两球质量均为 $m$，碰前速度 $\mathbf v_1,\mathbf v_2$，碰后 $\mathbf v_1',\mathbf v_2'$。动量守恒 $\mathbf v_1+\mathbf v_2=\mathbf v_1'+\mathbf v_2'$，动能守恒（完全弹性）$|\mathbf v_1|^2+|\mathbf v_2|^2=|\mathbf v_1'|^2+|\mathbf v_2'|^2$。

    把动量式两边平方：$|\mathbf v_1|^2+|\mathbf v_2|^2+2\mathbf v_1\!\cdot\!\mathbf v_2=|\mathbf v_1'|^2+|\mathbf v_2'|^2+2\mathbf v_1'\!\cdot\!\mathbf v_2'$。代入能量式消去平方和，得 $\mathbf v_1\!\cdot\!\mathbf v_2=\mathbf v_1'\!\cdot\!\mathbf v_2'$。

    转到第二球碰前静止的参照系（令 $\mathbf v_2=0$）：此时左边 $\mathbf v_1\!\cdot\!\mathbf v_2=0$，故 $\mathbf v_1'\!\cdot\!\mathbf v_2'=0$。若两球都未停住（一般情形），则 $\mathbf v_1'\perp\mathbf v_2'$，即等质量弹性碰撞后两球相互垂直分离。垂直关系是矢量关系，回到原参照系做伽利略变换仍可由同一恒等式表达。
...
## 18.7 相对论性能量和（角）动量 { .tutor-section }
...
??? question "答案 [18.16]"
    自旋的滑冰者绕竖直轴几乎不受外力矩（冰面摩擦与空气阻力可忽略），故对该轴的总角动量 $L=I\omega$ 守恒。收紧双臂使质量更靠近转轴，转动惯量 $I$ 减小；为保持 $L$ 不变，角速度 $\omega=L/I$ 必随之增大，于是转得更快。

    注意转动动能 $\tfrac12 I\omega^2=L^2/(2I)$ 反而增大，这部分能量来自滑冰者收臂时克服“离心”趋势所做的功，并不违反能量守恒。
...
??? question "答案 [18.19]"
    四维速度是世界线按固有时（弧长）$s$ 的导数 $v^a=\mathrm{d}x^a/\mathrm{d}s$，且 $v_av^a=c^2$。由于 $\mathrm{d}s^2=\mathrm{d}t^2(1-\mathbf v^2/c^2)$，有 $\mathrm{d}t/\mathrm{d}s=\gamma=(1-\mathbf v^2/c^2)^{-1/2}$，故 $v^a=\mathrm{d}x^a/\mathrm{d}s=\gamma\,\mathrm{d}x^a/\mathrm{d}t=\gamma(c^2,\mathbf v)$。

    四维动量 $p^a=\mu v^a$ 的空间分量便是 $\mu\gamma\mathbf v$。把它写成牛顿形式 $\mathbf p=m\mathbf v$，即要求 $m=\gamma\mu$；这正是相对论性（增）质量随速度增大的原因，$\gamma$ 因子来源于固有时与坐标时之比。
...
??? question "答案 [18.20]"
    设 $f(x)=(1+x)^{1/2}$。由泰勒展开 $f(x)=\sum_{n\ge0}\dfrac{f^{(n)}(0)}{n!}x^n$，逐项算导数：$f'=\tfrac12(1+x)^{-1/2}$，$f''=-\tfrac14(1+x)^{-3/2}$，$f'''=\tfrac38(1+x)^{-5/2}$，在 $x=0$ 取值得 $(1+x)^{1/2}=1+\tfrac12 x-\tfrac18 x^2+\tfrac1{16}x^3-\cdots$。

    能量 $E=(c^4\mu^2+c^2\mathbf p^2)^{1/2}=c^2\mu\big(1+\tfrac{\mathbf p^2}{c^2\mu^2}\big)^{1/2}$。令 $x=\mathbf p^2/(c^2\mu^2)$ 代入上式，得 $E=c^2\mu\Big(1+\tfrac12\tfrac{\mathbf p^2}{c^2\mu^2}-\tfrac18\tfrac{\mathbf p^4}{c^4\mu^4}+\tfrac1{16}\tfrac{\mathbf p^6}{c^6\mu^6}-\cdots\Big)$。

    逐项为：第一项 $c^2\mu$ 即静能量，正是 $E=mc^2$（此处 $m=\mu$）；第二项 $\dfrac{\mathbf p^2}{2\mu}$ 即牛顿动能 $\tfrac12\mu v^2$（低速 $\mathbf p\approx\mu\mathbf v$）。后续两项为相对论修正 $-\dfrac{\mathbf p^4}{8c^2\mu^3}+\dfrac{\mathbf p^6}{16c^4\mu^5}$，它们给出对完整相对论能量的更好估计。
