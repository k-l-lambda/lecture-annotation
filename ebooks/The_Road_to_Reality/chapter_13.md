...
??? question "答案 [13.7]"
    $O(3)$ 的非平凡正规子群必须在共轭下保持不变。旋转的共轭仍是同角度、但绕变换后轴的旋转；若正规子群含任一非平凡一般旋转，则因轴可由共轭任意改变并由旋转角组合生成全部 $SO(3)$，故含 $SO(3)$。若它还含一个反射，则乘以 $SO(3)$ 给出整个 $O(3)$。在正文讨论的连续转动不变量意义下，唯一非平凡真正规子群是 $SO(3)$。
